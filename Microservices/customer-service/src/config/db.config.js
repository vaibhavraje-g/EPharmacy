...
const connectDB = async () => {
  try {
    ...
    if (!config || !config.DataSource || !config.DBHost || !config.DBPort || !config.customerDB) {
      throw new Error('Invalid configuration. Missing required fields.');
    }

    const connectionString = `${config.DataSource}://${config.DBHost}:${config.DBPort}/${config.customerDB}`;
    ...
    console.log(`DB connected successfully to ${config.customerDB}`);
  } catch (err) {
