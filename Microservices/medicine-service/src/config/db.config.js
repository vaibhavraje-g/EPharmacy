...
const connectDB = async () => {
  try {
    ...
    if (!config || !config.DataSource || !config.DBHost || !config.DBPort || !config.medicineDB) {
      throw new Error('Invalid configuration. Missing required fields.');
    }

    const connectionString = `${config.DataSource}://${config.DBHost}:${config.DBPort}/${config.medicineDB}`;
    ...
    console.log(`DB connected successfully to ${config.medicineDB}`);
  } catch (err) {
