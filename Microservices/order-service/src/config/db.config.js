...
const connectDB = async () => {
  try {
    ...
    if (!config || !config.DataSource || !config.DBHost || !config.DBPort || !config.orderDB) {
      throw new Error('Invalid configuration. Missing required fields.');
    }

    const connectionString = `${config.DataSource}://${config.DBHost}:${config.DBPort}/${config.orderDB}`;
    ...
    console.log(`DB connected successfully to ${config.orderDB}`);
  } catch (err) {
