...
const connectDB = async () => {
  try {
    ...
    if (!config || !config.DataSource || !config.DBHost || !config.DBPort || !config.paymentsDB) {
      throw new Error('Invalid configuration. Missing required fields.');
    }

    const connectionString = `${config.DataSource}://${config.DBHost}:${config.DBPort}/${config.paymentsDB}`;
    ...
    console.log(`DB connected successfully to ${config.paymentsDB}`);
  } catch (err) {
